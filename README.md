...
> 2026-09-20 当前个人服修改：管理地址改为 `http://192.168.123.66/`（80 端口）；r24 精简开局选项，经验/金钱基础倍率在比赛 5～15 分钟由 1× 线性升至 1.5×，AI 另叠加死亡层数、最高 5×；选人和出兵前各固定 45 秒。攻击间隔按钮限制为初始值 ±0.3 秒，聊天 `-basetime 1.7` 可直接设置正数秒值。源码、编译与实际部署状态以 [最新现场记录](docs/AI_FUN_ADAPTATION.md) 为准；下面的早期版本说明保留了已经移除的机器人管理及选单流程。
