...
base='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Dota 2 LAN · 使用帮助</title>{style}</head><body><main class="help-shell"><p class="eyebrow">DOTA 2 LAN KIT / DOCUMENTATION / V1.0.0</p><a href="/">← 返回控制中心</a>{body}<footer>本地自托管 · 真实联机能力按现场验收确认</footer></main></body></html>'
(ROOT/'panel/static/help.html').write_text(base.replace('{style}','<link rel="stylesheet" href="/style.css">').replace('{body}',body))
css=(ROOT/'panel/static/style.css').read_text()
(ROOT/'docs/HELP.html').write_text(base.replace('{style}','<style>'+css+'</style>').replace('{body}',body).replace('<a href="/">← 返回控制中心</a>','<p class="muted">离线帮助 · 完整部署步骤见同目录 DEPLOYMENT.md</p>'))
