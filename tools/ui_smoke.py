...
def main():
    ...
    def offline_html(page, name):
        html = (static/name).read_text().replace('<link rel="stylesheet" href="/style.css">','').replace('<script src="/app.js" defer></script>','')
    ...
    with sync_playwright() as pw:
        ...
        results.append('Unauthenticated login page renders')
        page.locator('#loginPass').fill('not-the-password')
        page.locator('#loginForm button').click()
        expect(page.locator('#loginError')).to_contain_text('账号或密码不正确')
        results.append('Wrong password rejected and displayed')
        page.locator('#loginPass').fill('demo-local-only-12345')
        page.locator('#loginForm button').click()
        ...
        results.append('Authenticated dashboard shows real fake-process status / test Build ID')
        if args.offline_transport:
        ...
        results.append('Authenticated diagnostics download is valid JSON')
        ...
        expect(page.locator('#loginScreen')).to_be_visible()
        assert (http_call('/api/status')['status'] if args.offline_transport else page.request.get(base+'/api/status').status) == 401
        results.append('Logout invalidates session')
