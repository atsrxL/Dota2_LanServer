...
IMPORTANT: user is playing; **server source staged only**, active match preserved at r22 source b48a9fd460c995df46082c91d7bdd1297e3c16d084c517c13a4eeeab4f6fd667. Manual panel restart/new server start required to deploy r23; automatic crash retry source-lock may reject changed source. Client Dota restart clears Panorama cache. Actual r23 in-match behavior still unverified. Server source backup /root/agent.backup/dota-r23-source-20260919/before-r23.tar.gz. Client backups.98 20260919-173455 and.180 20260919-173431; known older snapshots rotated to retain latest2. No public push at that time (r23 subsequently pushed to main on user request).

## r24 — time-based income, simplified setup and panel port80 (2026-09-20)

Removed both sides' initial-gold and gold/XP multiplier controls and server options. No spawn-time gold overwrite: the game supplies its default starting gold. Selection/pregame each fixed45 seconds in server validation, hidden from UI. Both human and bot positive gold/XP now use the actual match clock (`GameRules:GetDOTATime(false,false)`): base1x until300s, linear to1.5x at900s, fixed1.5x thereafter. Pre-game and pauses do not advance this curve. Existing independent bot death layers add to this base, enemy hero kill reduces3 layers, bot final multiplier capped5x. Fractional rewards carried for both human and bots; spending, direct tool gold and direct level grants do not scale.

BAT panel changes remain0.1-second steps but are limited to original BAT±0.3. Reset restores original BAT even after command changes. Added engine player_chat `-basetime <positive decimal seconds>` with identity/cheat checks and feedback, no panel-range restriction; rejects invalid/nonfinite/unrepresentable values rather than evaluating console text. Removed Beastmaster shortcut; six shortcuts ordered death_prophet_witchcraft, razor_unstable_current, bloodseeker_thirst, winter_wyvern_eldwurms_edda, tinker_eureka, silencer_brain_drain. Typed non-innate abilities remain level0.

Full212 tests passed (28 existing warnings), including clock0/5/10/15/30min, fractional human income, bot ceiling,45s engine calls, rejected removed options, BAT endpoints/overflow/reset, command outside panel bounds and shortcut order.

Live panel migrated8443->80: nginx site and **only CT270** PVE firewall management rule changed; preserved LAN source192.168.123.0/24 and gameUDP27015. Plan reviewed, nginx syntax and local/external HTTP API passed. Backups on affected guest/host under `/root/agent.backup/dota-panel-port80-20260920/`. Installer examples/defaults now80 and validators allow80 specifically for panel (game port still1024+). Addon management page retained pending user's decision; recommended consolidating its read-only resource/error/AI evidence into overview/diagnostics while keeping addon runtime enabled.

Compilation/deployment initially blocked: Tiny11_clone was shut off; `virsh start` hangs before QEMU launch, even bounded read-only libvirt queries time out. Znas SSH works; no host service restart or hidden configuration mutation performed. .180 SSH connection times out. Server stopped with no active job. New UI not yet compiled, r24 game source not yet installed; existing r23 compiled resources must not be labeled r24. Continue from here once Windows Tools becomes reachable.

2026-09-20 resumed after UI rollback: preserved existing r24 edits. Removed standalone addon navigation/form/launch strategy/deploy controls. Overview now retains read-only resource/heartbeat/error summary plus diagnostic download; backend addon remains required and enabled. Published ONLY three web static files to CT270; live HTTP80 readback confirms addonForm absent and addonSummary present. Backup /root/agent.backup/dota-r24-web-20260920/before.tar.gz. Game logic suite212 passed; browser smoke unavailable (Playwright missing), JS syntax passed. QGA bounded query timed out and .180 SSH timed out. r24 UI still uncompiled and game deployment pending; no restart or mismatched resource release.
